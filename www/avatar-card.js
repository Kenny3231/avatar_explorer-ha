...
class AvatarCard extends LitElement {
  ...
  setConfig(config) {
    ...
    // Sélection du profil par défaut à l'initialisation
  ...
  // Scan des entités dynamiques pour récupérer le dossier "directory"
  _getUsersFromHass() {
    if (!this.hass) return [];
  ...
  static get styles() {
    return css`
      :host { --primary-blue: #0A84FF; --card-bg: #1C1C1E; }
      ha-card { background: var(--card-bg); padding: 16px; color: white; border-radius: 24px; border: 1px solid rgba(255,255,255,0.1); }
      ...
      select, input { width: 100%; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; padding: 12px; color: white; font-weight: bold; outline: none; box-sizing: border-box; }
      
      .grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 12px; max-height: 500px; overflow-y: auto; }
      .grid::-webkit-scrollbar { width: 0px; }
      .item { background: rgba(255,255,255,0.05); border-radius: 20px; padding: 10px; text-align: center; cursor: pointer; transition: 0.2s; border: 1px solid transparent; }
      .item:hover { transform: translateY(-5px); border-color: var(--primary-blue); }
      .item img { width: 100%; aspect-ratio: 1; object-fit: contain; }
      .item span { display: block; font-size: 8px; font-weight: 900; text-transform: uppercase; color: #888; margin-top: 5px; }
      
      .lightbox { position: fixed; inset: 0; background: rgba(0,0,0,0.9); z-index: 9999; display: flex; align-items: center; justify-content: center; backdrop-filter: blur(10px); }
      .modal { position: relative; background: #1C1C1E; padding: 30px; border-radius: 32px; width: 90%; max-width: 450px; text-align: center; border: 1px solid rgba(255,255,255,0.1); }
      .modal img { max-height: 200px; margin-bottom: 20px; }
      
      .close-btn { position: absolute; top: 15px; right: 15px; background: none; border: none; color: #888; font-size: 28px; cursor: pointer; line-height: 1; }
      ...
      .btn { padding: 12px; border-radius: 15px; border: none; font-weight: 800; cursor: pointer; font-size: 10px; text-transform: uppercase; }
...
class AvatarCardEditor extends LitElement {
  ...
  static get styles() {
    return css`
      ...
      .option-row { display: flex; align-items: center; justify-content: space-between; background: rgba(0,0,0,0.05); padding: 10px; border-radius: 8px; }
      .option-row input { padding: 8px; border-radius: 5px; border: 1px solid #ccc; width: 50%; }
      .users-section { background: rgba(0,0,0,0.02); padding: 10px; border-radius: 8px; border: 1px solid #ccc; }
      .user-row { display: flex; align-items: center; gap: 10px; margin-bottom: 10px; background: white; padding: 10px; border-radius: 8px; border: 1px solid #eee; color: black; }
      .folder-id { font-size: 11px; color: #666; font-weight: bold; margin-bottom: 4px; }
      ...
      .del { cursor: pointer; color: #ff453a; }
      .add-select { width: 100%; padding: 12px; background: var(--primary-color); color: white; border: none; border-radius: 8px; cursor: pointer; margin-top: 10px; font-weight: bold; }
    `;
  }
}
